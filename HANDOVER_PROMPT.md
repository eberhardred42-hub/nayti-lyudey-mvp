...
2) Бриф собирается детерминированно максимум за 10 вопросов; LLM используется только для extract/sanity (и должно быть проверяемо через `/api/health/llm/ping`).
...
Текущее место истины по интро:
- движок: [api/intro_engine.py](api/intro_engine.py)
- ручной E2E смоук на DEV: workflow “Intro Flow Smoke (DEV)” ([.github/workflows/intro-flow-smoke-dev.yml](.github/workflows/intro-flow-smoke-dev.yml))
