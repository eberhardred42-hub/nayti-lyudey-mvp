...
- Минимум:
  - `bash scripts/tests/test_persistence_postgres.sh`
  - `bash scripts/tests/test_observability_request_ids.sh`
  - `bash scripts/tests/test_parsing_contracts.sh`
  - `bash scripts/tests/test_free_report_generation.sh`
  - `bash scripts/tests/test_pdf_pipeline.sh`
...
2) End-to-end рендер
- Запустить `bash scripts/smoke-staging.sh <DOMAIN>` (на стенде) либо локально `bash scripts/tests/test_pdf_pipeline.sh`
- Убедиться, что:
  - создаётся pack
