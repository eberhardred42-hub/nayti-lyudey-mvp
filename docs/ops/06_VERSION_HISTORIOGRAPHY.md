...
### v3.0 — P0 Brief LLM loop (1 вопрос → 1 ответ → следующий)
- Фокус: сделать первый реальный LLM-цикл в режиме “Нет текста — отвечу на вопросы”.
- Что сделали: `/chat/message` для entry_mode=C генерирует следующий вопрос через `generate_questions_and_quick_replies` и возвращает `llm_used`; UI показывает бейдж `LLM`.
- Приёмка: чекпоинты A/B/C из README (LLM ping + 2 последовательных вопроса с `llm_used=true`).
