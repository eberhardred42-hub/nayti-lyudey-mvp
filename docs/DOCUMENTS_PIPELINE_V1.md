...
Интро сейчас поддерживает минимальный A/B/C/D сценарий. Для смоука можно выбрать B:
- отправить `intro_message` с текстом `B — своими словами`
- затем отправить одной строкой вводные (роль/формат/занятость/бюджет)
- затем `intro_message` = `Да, всё верно`
...
Формат ответа `generate_pack`:
- `results[]`: `{doc_id, title, status: cached|generated|failed, artifact_id, url, error?}`
...
- Промпты для документов лежат в `api/prompts/docs/<doc_id>/system.md` и `user_template.md`.
- Для бесплатного auto_generate-пака добавлены per-doc шаблоны, чтобы минимизировать reliance на fallback.
